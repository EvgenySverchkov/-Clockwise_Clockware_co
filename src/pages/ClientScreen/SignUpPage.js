...
function RegistrationForm({ history }) {
  ...
  function submitHandler(e) {
    ...
    if (elem.name.value.match(/\d/) || !elem.name.value.match(/\b\w{3,20}\b/)) {
      context.openWarningTooltip("String name should:\n1. Not contain numbers\n2. Not be shorter than 3 characters\n3. Not longer than 20 characters\n4. Do not contain Cyrillic characters!", elem.name.id);
      return false;
    }
    if (elem.email) {
      if (!elem.email.value.match(/^\w+@[a-zA-Z_0-9]+?\.[a-zA-Z]{2,}$/)) {
        context.openWarningTooltip("Invalid email format. Please check your email!", elem.email.id);
        return false;
      }
      if (elem.email.value.toLowerCase().match(/admin/)) {
        context.openWarningTooltip("Your email cannot contain the word 'admin'", elem.email.id);
        return false;
      }
    }
    if (elem.password.value.length < 4 || elem.password.value.length > 16) {
      context.openWarningTooltip("Password must not be less than 4 characters and must not be longer than 16 characters!", elem.password.id);
      return false;
    }
    ...
    fetch(`${SERVERDOMAIN}/signUp`, {
      ...
      .then((data) => {
        ...
        if (!data.success) {
          ...
          context.openSuccessWindowWithMsg(`Congratulations! ${data.user.name} you are registered`);
          elem.reset();
        }
      });
  }
