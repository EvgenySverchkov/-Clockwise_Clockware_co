...
import {changeWarningModalData, changeSuccessModalData} from "../../store/clientModalWindows/actions";
...
const UserSettings = () => {
    ...
    function submitHandler(e){
        ...
        if(obj.name.length<3){
            dispatch(changeWarningModalData({ msg: "Name must be at least 3 characters" }));
            document.getElementById("callWarningModalBtn").click();
            return false;
          }
        if (obj.name.match(/\d/)) {
            dispatch(changeWarningModalData({ msg: "The string name must not contain numbers!" }));
            document.getElementById("callWarningModalBtn").click();
            return false;
        }
    ...
    <form onSubmit = {submitHandler} className="mt-4 row justify-content-center">
        ...
        <FormGroup>
            ...
            <Name id={"userName"} value={userNameState} chngHandler={changeHandler}/>
        </FormGroup>
